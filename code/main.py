##### ADAPTED FROM https://github.com/cal-pratt/SheetVision

def jpg2pitches(filePath):
    import sys
    import subprocess
    import cv2
    import time
    ...
    # er_files = [
    #     "resources/template/eight-rest.png"
    # ]
    # wr_files = [
    #     "resouces/template/whole-rest.png"
    # ]
    ##### END MY CODE
    ...
    ##### START MY CODE
    bar_imgs = [cv2.imread(bar_file, 0) for bar_file in bar_files]
    # er_imgs = [cv2.imread(er_file, 0) for er_file in er_files]
    # wr_imgs = [cv2.imread(wr_file, 0) for wr_file in wr_files]
    ##### END MY CODE
    ...
    ##### START MY CODE
    bar_lower, bar_upper, bar_thresh = 50, 150, 0.8
    # er_lower, er_upper, er_thresh = 50, 150, 0.8
    # wr_lower, wr_upper, wr_thresh = 50, 150, 0.8
    ##### END MY CODE
    
    # finds the images in the jpg using cv
    ...
    # merges results that are the same
    ...
    if __name__ == "__main__" or __name__ == "main":
        ...
        ##### START MY CODE
        print("Matching bar image...")
        bar_recs = locate_images(img_gray, bar_imgs, bar_lower, bar_upper, bar_thresh)
        
        print("Merging bar image results...")
        bar_recs = merge_recs([j for i in bar_recs for j in i], 0.5)
        bar_recs_img = img.copy()
        for r in bar_recs:
            r.draw(bar_recs_img, (0, 0, 255), 2)
        cv2.imwrite('bar_recs_img.png', bar_recs_img)
        # open_file('bar_recs_img.png')
        ##### END MY CODE
    
        note_groups = []
        for box in staff_boxes:
            staff_sharps = [Note(r, "sharp", box) 
                # if the sharp is located within the staff
            ...
            ##### START MY CODE
            bar_rests = [Rest(r, "4", box)
                for r in bar_recs if abs(r.middle[1] - box.middle[1]) < box.h*5.0/8.0]
            staff_notes = quarter_notes + half_notes + whole_notes + bar_rests
            ##### END MY CODE
        ...
        for note_group in note_groups:
            if len(note_group) == 1:
                for note in note_group:
                    if str(note.sym) == "2":
                        # half note
                        pitches.append((str(note.pitch), str(note.sym)))
                    else:
                        # quarter note
                        pitches.append((str(note.pitch), "4"))
            else:
                # eighth note
                for note in note_group:
                    pitches.append((str(note.pitch), "8"))
            # print([ str(note.pitch) + " " + str(note.sym) for note in note_group])
            # pitches.append([ str(note.pitch) + " " + str(note.sym) for note in note_group])
